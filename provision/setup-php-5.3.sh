...
# Load settings.yaml
eval $(parse_yaml /vagrant/config/settings-php53.yaml "config_")
...
libapache2-mod-fastcgi > /dev/null 2>&1
  ...
  wget --quiet -c -t 3 -O ./php-5.3.29.tar.gz http://museum.php.net/php5/php-5.3.29.tar.gz
...
./configure \
  ...
  mkdir /etc/php53/conf.d
  cp php.ini-recommended /etc/php53/apache2/php.ini
...
"
  ...
  echo "$script_cgi" > "/usr/lib/cgi-bin/php53-cgi"
  chmod +x /usr/lib/cgi-bin/php53-cgi

  apache2_php53="# Include file for virtual hosts that need to run PHP 5.3
...
"
  ...
  echo "$apache2_php53" > "/etc/apache2/php53.conf"

  add_line "Include php53.conf" "/etc/apache2/apache2.conf"
  rm -rf /tmp/php-5.3.29*

  ln -s /usr/bin/php53/php /usr/bin/php
}
...
package_install() {
  ...
  show_message "Installing PHP 5.3.29..."
  install_php53
}
...
echo "Provisioning VM PHP 5.3 complete"
