...
# PHP
if [ ! -f "/vagrant/config/php-5.3/php-cli.ini" ]; then
  contents=$(< /vagrant/provision/templates/php-5.3/php-cli.ini)
  contents=$(echo "$contents" | sed -e "s@\$TIMEZONE@$config_timezone@g")
  echo "$contents" > /vagrant/config/php-5.3/php-cli.ini
fi
cp /vagrant/config/php-5.3/php-cli.ini /etc/php53/conf.d/99-custom.ini
