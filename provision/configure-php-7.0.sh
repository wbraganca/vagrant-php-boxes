...
# PHP CLI
if [ ! -f "/vagrant/config/php-7.0/php-cli.ini" ]; then
  contents=$(< /vagrant/provision/templates/php-7.0/php-cli.ini)
  contents=$(echo "$contents" | sed -e "s@\$TIMEZONE@$config_timezone@g")
  echo "$contents" > /vagrant/config/php-7.0/php-cli.ini
fi
cp /vagrant/config/php-7.0/php-cli.ini /etc/php/7.0/cli/conf.d/99-custom.ini
chmod 644 /etc/php/7.0/cli/conf.d/99-custom.ini


# PHP FPM
if [ ! -f "/vagrant/config/php-7.0/php-fpm.ini" ]; then
  contents=$(< /vagrant/provision/templates/php-7.0/php-fpm.ini)
  contents=$(echo "$contents" | sed -e "s@\$TIMEZONE@$config_timezone@g")
  echo "$contents" > /vagrant/config/php-7.0/php-fpm.ini
fi
cp /vagrant/config/php-7.0/php-fpm.ini /etc/php/7.0/fpm/conf.d/99-custom.ini
chmod 644 /etc/php/7.0/fpm/conf.d/99-custom.ini


# nginx
if [ ! -f "/vagrant/config/php-7.0/nginx/nginx.conf" ]; then
  cp /vagrant/provision/templates/php-7.0/nginx/nginx.conf /vagrant/config/php-7.0/nginx/nginx.conf
  rm -rf /etc/nginx/nginx.conf
fi
cp /vagrant/config/php-7.0/nginx/nginx.conf /etc/nginx/nginx.conf
